...
@section('content')
    <style>
        ...
        .kpi-card:hover {
            ...
            box-shadow: 0 10px 25px rgba(0, 0, 0, 0.06), 0 4px 8px rgba(0, 0, 0, 0.08)
        }
    ...
    <div class="alert alert-light border-0 shadow-sm mb-4">
        <div class="d-flex justify-content-between align-items-start">
            <div>
                <div class="h4 mb-1">Selamat Datang, {{ $santri->nama }}! 👋</div>
                ...
                <div class="fst-italic text-secondary" style="font-size: 13px;">
                    💡 <strong>Motivasi Hari Ini:</strong> "Konsistensi adalah kunci kesuksesan. Terus semangat dalam menghafal Al-Qur'an!"
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        @php
            $kpiItems = [
                ...
                [
                    'label' => 'Hadir Tidak Setor',
                    'value' => $totalHadirTidakSetor,
                    'sub' => 'Akumulasi kehadiran tanpa setoran',
                    'color' => 'warning',
                ...
                <div class="card kpi-card border-0 h-100">
                    <div class="card-body">
                        ...
                        <div class="kpi-progress mt-3">
                            <div class="kpi-progress-bar bg-{{ $item['color'] }}"
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            ...
            <div class="text-muted small mt-2">Overall dihitung berdasarkan tahap tertinggi yang telah diselesaikan per juz.
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
                    ...
                    @foreach ($progressPerJuz as $p)
                        @if ($p['pct'] > 0 || $p['tahap'])
                            <div class="enterprise-progress-row" data-coreui-toggle="tooltip"
                                title="Juz {{ $p['juz'] }} • Progress: {{ $p['pct'] }}% • Status: {{ $p['status'] }}@if ($p['tahap']) • Tahap: {{ ucfirst(str_replace('_', ' ', $p['tahap'])) }} @endif">
                                <div class="enterprise-progress-header">
                                    <div class="enterprise-juz">Juz {{ $p['juz'] }}</div>
                                    <div class="enterprise-meta">
                                        @if ($p['tahap'])
                                            <span
                                                class="badge bg-light text-dark border">{{ ucfirst(str_replace('_', ' ', $p['tahap'])) }}</span>
                                        @endif
        ...
        <div class="col-lg-6">
            <div class="card h-100">
                ...
                <div class="card-body"><canvas id="chartJuzPct" height="170"></canvas></div>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Enterprise progress bars animation
            document.querySelectorAll('.enterprise-progress .progress-bar').forEach((bar, index) => {
