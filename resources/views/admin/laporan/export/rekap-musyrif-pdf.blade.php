...
<body>
    <section class="hero">
        <table class="hero-table">
            <tr>
                <td>
                    <h1 class="hero-title">Rekap Hafalan per Musyrif</h1>
                    <p class="hero-subtitle">Export PDF analitik musyrif dengan pemetaan 30 Juz, cakupan setoran, dan capaian ujian akhir.</p>
                </td>
    ...
    <section class="section">
        ...
        <table class="summary">
            <tr>
                <td class="card-purple"><div class="label">Total Musyrif</div><div class="value">{{ $summary['total_musyrif'] ?? 0 }}</div><div class="note">Musyrif sesuai filter.</div></td>
                <td class="card-blue"><div class="label">Santri Binaan</div><div class="value">{{ $summary['total_santri'] ?? 0 }}</div><div class="note">Santri dalam placement semester.</div></td>
                <td class="card-green"><div class="label">Setoran Harian</div><div class="value">{{ $summary['total_setoran_harian'] ?? 0 }}</div><div class="note">Harian, tahap 1, tahap 2, tahap 3.</div></td>
            ...
            <tr>
                <td><div class="label">Santri Aktif</div><div class="value">{{ $summary['santri_aktif_setoran'] ?? 0 }}</div><div class="note">Punya setoran atau ujian.</div></td>
                <td><div class="label">Santri Sudah Ujian</div><div class="value">{{ $summary['santri_sudah_ujian'] ?? 0 }}</div><div class="note">Minimal 1 Juz lulus ujian.</div></td>
                <td><div class="label">Rata Nilai Ujian</div><div class="value">{{ number_format((float) ($summary['avg_nilai_ujian'] ?? 0), 2) }}</div><div class="note">Hanya dari ujian akhir.</div></td>
                <td><div class="label">Alpha</div><div class="value">{{ $summary['total_alpha'] ?? 0 }}</div><div class="note">Perlu follow-up.</div></td>
            </tr>
        </table>
    </section>
    ...
    <section class="section page-break">
        <h2 class="section-title">Data Detail Musyrif</h2>
        <table class="data">
            <thead>
                <tr>
                    <th>No</th>
            ...
            <tbody>
                @foreach ($data as $row)
                    <tr>
                        ...
                        <td class="text-right">{{ (int) ($row->santri_aktif_setoran ?? 0) }}</td>
                        <td class="text-right">{{ (int) ($row->jumlah_setoran_harian ?? 0) }}</td>
                        <td class="text-right">{{ (int) ($row->jumlah_ujian ?? 0) }}</td>
                        <td class="text-right">{{ number_format((float) ($row->coverage_ujian_pct ?? 0), 1) }}%</td>
